...
### 7.1 Transparence "Glass Box" — Tool Chain (REQ_AUDIT_060)
**Catégorie :** Audit / Security.

**REQ_AUDIT_060 (Transparent Tool Chain) :**
Toute exécution locale orchestrée par le wrapper au titre d’“inspection”/audit (ou tout autre besoin de ground truth) DOIT suivre un protocole **Artifact-First** garantissant une transparence totale :

1) **Script avant exécution** : tout code généré par l’IA pour exécution locale DOIT être sauvegardé comme artefact permanent (fichier `.py`) **AVANT** l’exécution.
